...
import com.skye.yunpicturebackend.api.aliyunai.AliYunAiApi;
...
import com.skye.yunpicturebackend.manager.upload.FilePictureUpload;
import com.skye.yunpicturebackend.manager.upload.PictureUploadTemplate;
import com.skye.yunpicturebackend.manager.upload.UrlPictureUpload;
...
import com.skye.yunpicturebackend.model.enums.PictureReviewStatusEnum;
...
import com.skye.yunpicturebackend.mapper.PictureMapper;
...
//            }
        ...
        //开启事务
        //更新空间的使用额度(由于要操作插入图片表和修改空间表的数据，同时做两个操作，必须得用事务了)
        ...
        transactionTemplate.execute(status -> {
            ...
            if (finalSpaceId != null) {
                boolean update = spaceService.lambdaUpdate()
                        //Space::getId：负责告诉数据库要操作“哪一列”（即 id 这个字段）。
                        //finalSpaceId：负责告诉数据库这一列要匹配的“具体值”（即具体的 ID 号码）
                        .eq(Space::getId, finalSpaceId)
                        //setSql 方法的作用是让你直接拼接一段自定义的 SQL 片段到 SET 语句中
                        //类似UPDATE space SET total_size = total_size + picture.getPicSize(), total_count = total_count + 1 WHERE id =
    ...
    //下面都是对应增删改查的方法补充，但是写的都是通用类代码
    //因为增删改查业务逻辑比较简单，我们直接写在controller中，再调用这些方法
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        //因为之后要判断参数reviewStatus的正确性，所以我们用枚举类吧参数包一层
        ...
        //数据库操作
        Picture updatePciture = new Picture();
        BeanUtil.copyProperties(pictureReviewRequest,updatePciture);
        updatePciture.setReviewerId(loginUser.getId());
        updatePciture.setReviewTime(new Date());
        boolean result = this.updateById(updatePciture);
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        ThrowUtils.throwIf(pictureIdList == null && spaceId == null, ErrorCode.PARAMS_ERROR);
        ThrowUtils.throwIf(loginUser == null, ErrorCode.NO_AUTH_ERROR);
        ...
        // 会把 pictureList 里的每一个 Picture 对象依次拿出来，交给大括号 {} 里的代码去处理
        pictureList.forEach(picture -> {
